    ...
    stages {
        ...
        stage('Publish') {
            steps {
                ...
                sh 'npm config set email=hectoralejandro2121@gmail.com && npm config set _auth NpmToken.97519605-bd0c-3d21-ae38-e58420da0e27 && npm config set registry http://192.168.0.7:8081/repository/npm-group/ && npm config set always-auth=true'
            }
        }
        stage('Deploy') {
            steps {

    ...
    stages {
        ...
        stage('Publish') {
            steps {
                ...
                sh 'npm-cli-login -u jenkins-user -p 12345 -e hectoralejandro2121@gmail.com -r http://192.168.0.7:8081/repository/npm-group/'
                ...
                sh 'npm config set email=hectoralejandro2121@gmail.com && npm config set _auth amVua2lucy11c2VyOjEyMzQ1 && npm config set registry http://192.168.0.7:8081/repository/npm-group/ && npm config set always-auth=true && echo 192.168.0.7:8081/:_authToken=NpmToken.97519605-bd0c-3d21-ae38-e58420da0e27 ~/.npmrc && npm whoami --registry=http://192.168.0.7:8081/repository/npm-group/ && npm --registry=http://192.168.0.7:8081/repository/npm-group/ publish'
            }
        }
        stage('Deploy') {
            steps {
